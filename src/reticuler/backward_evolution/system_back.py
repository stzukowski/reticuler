...
class BackwardBifurcations:
    ...
    def bif_info(self):
        ...
        return np.column_stack(
            (self.mother_IDs, self.flux_info, self.length_mismatch, self.flags)
        )
...
class BackwardSystem:
    ...
    def __init__(
        ...
        self.backward_branches, self.backward_bifurcations = self.import_branches(
            self.system.network
        )
        self.system.extender.pde_solver.eta = self.trimmer.eta
        self.system.extender.pde_solver.crit_shields_param = (
            self.trimmer.crit_shields_param
        )
    ...
    def import_json(cls, input_file, system):
        ...
        for i in reversed(list(json_load["backward_branches"].keys())):
            ...
            if points_numsleft_steps.size:
                ...
                backward_branch.nums_left = np.array(
                    points_numsleft_steps[:, 2], dtype=int
                )
    ...
    def import_branches(self, network):
        ...
        for i, forward_branch in enumerate(network.branches):
            network.branch_connectivity[branch_connectivity_0 == forward_branch.ID] = i
            network.box.seeds_connectivity[
                seeds_connectivity_0[:, 1] == forward_branch.ID, 1
            ] = i
            forward_branch.ID = i
            ...
            if network.branch_connectivity.size != 0 and np.isin(
                forward_branch.ID, network.branch_connectivity[:, 0], invert=True
            ):
                network.active_branches.append(forward_branch)
    ...
    def __compare_networks(
        self, initial_network, backward_network, test_network, flux_info
    ):
        ...
        for i in range(len(initial_network.active_branches)):
            backward_branch = self.backward_branches[
                initial_network.active_branches[i].ID
            ]
            bifurcation_ind = (
                self.backward_bifurcations.mother_IDs == backward_branch.mother_ID
            )
            ...
            # (if we've reached it only once then the branch is popped from active_branches)
                ...
                # angular deflection
                backward_branch.angular_deflection = np.append(
                    backward_branch.angular_deflection, real_angle - test_angle
                )
                # a1, a2, a3 coefficients
                backward_branch.flux_info = np.vstack(
                    (backward_branch.flux_info, flux_info[i])
                )
    ...
    def run_BEA(self):
        ...
        while (
            not len(self.system.network.branches) == 1
            and self.BEA_step < self.BEA_step_thresh
        ):
            self.BEA_step = self.BEA_step + 1
            logger.info("---------------------------------------------------")
            logger.info(
                "----- Backward Evolution Algorithm step: %.0f -----", self.BEA_step
            )
