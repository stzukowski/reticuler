...
class BackwardModifiedEulerMethod:
    ...
    def __init__(
        ...
        self.min_distance = (
            min_distance  # avoid two nodes very close to each other after trimming
        )
    ...
    def __explicit_network_trim(
        self, test_network, drs_0, backward_branches, backward_bifurcations, BEA_step
    ):
        ...
        dts_to_bif = np.zeros(
            (len(backward_bifurcations.mother_IDs), len(branches_to_iterate))
        )
        for i, forward_branch in enumerate(branches_to_iterate):
            ...
            ind_bifurcation = (
                backward_bifurcations.mother_IDs == backward_branch.mother_ID
            )
        ...
        are_bifs_reached_second_time = np.logical_or(
            counts_reached == 2,
            np.logical_and(counts_reached == 1, backward_bifurcations.flags == 1),
        )
        ...
        for i, forward_branch in enumerate(branches_to_iterate):
            ...
            if drs[i] + self.min_distance >= length_from_tip[-1]:
                ...
                test_network.branch_connectivity = test_network.branch_connectivity[
                    mask, ...
                ]
                ...
                if backward_branch.mother_ID == -1:
                    mask = (
                        test_network.box.seeds_connectivity[:, 1] != forward_branch.ID
                    )
                    test_network.box.seeds_connectivity = (
                        test_network.box.seeds_connectivity[mask, ...]
                    )
                ...
                # bifurcation point
                    ...
                    if backward_bifurcations.flags[ind_bifurcation] == 0:
                        logger.info(
                            "! Branch %d reached bifurcation %d ! (1st one)",
                            backward_branch.ID,
                        ...
                        remaining_length, remaining_IDs = (
                            self.__calculate_remaining_length(
                                test_network, backward_branch.mother_ID
                            )
                        )
                        inds_remaining = [
                            i
                            for i, b in enumerate(branches_to_iterate)
                            if b.ID in remaining_IDs
                        ]

                        # if drs[i]>0 when tip(i) reaches the bifPoint, then dt was too big;
                        # we compansate the effect by adding the 'virtual length'
                        virtual_length = (
                            drs[i] / drs_0[i] * np.sum(drs_0[inds_remaining])
                        )
                        ...
                        if length_mismatch < 0:
                            virtual_length = (
                                (np.sum(drs[inds_remaining]) - remaining_length)
                                / np.sum(drs_0[inds_remaining])
                                * drs_0[i]
                            )
                            length_mismatch = virtual_length - drs[i]

                        backward_bifurcations.length_mismatch[ind_bifurcation] = (
                            length_mismatch
                        )
                        drs[i] = 0
                        ...
                        logger.info(
                            "! Branch %d reached bifurcation %d ! (2nd one)",
                            backward_branch.ID,
                            backward_branch.mother_ID,
                        )
                        ...
                        mother_branch = [
                            b
                            for b in test_network.branches
                            if b.ID == backward_branch.mother_ID
                        ][0]
            ...
            # bifurcation not reached
                ...
                if drs[i] != 0:
                    ...
                    forward_branch.points[-1] = (
                        forward_branch.points[-1] - tip_versor * drs[i]
                    )
                    backward_branch.add_point(
                        forward_branch.points[-1],
                        len(forward_branch.points) - 1,
                        BEA_step,
                    )
                    drs[i] = 0
        ...
        if (counts_reached == 2).any():
            ...
            for i, forward_branch in enumerate(branches_to_iterate):
                if forward_branch not in test_network.active_branches:
                    ...
                    inds_after_bif[i] = test_network.active_branches.index(
                        mother_branch
                    )
                else:
                    inds_after_bif[i] = test_network.active_branches.index(
    ...
    def __calculate_remaining_length(self, network, mother_ID):
        to_check = network.branch_connectivity[
            network.branch_connectivity[:, 0] == mother_ID, 1
        ]
        ...
        while to_check.size:
            ...
            remaining_length = (
                remaining_length
                + network.branches[np.where(branch_IDs == next_ID)[0][0]].length()
            )

            to_add = network.branch_connectivity[
                network.branch_connectivity[:, 0] == next_ID, 1
            ]
            if to_add.size:
    ...
    def trim(self, network, backward_branches_0, backward_bifurcations_0, BEA_step):
        ...
        # are moving?
        are_moving = self.__check_moving_conditions(
            network
        )  # here we put branches to sleep
        if not are_moving.any():
            raise Exception(
                "Not a single branch is moving. The critical Shields parameter might be too high."
            )
        velocity_0 = (
            self.pde_solver.flux_info[are_moving, 0] - self.crit_shields_param
        ) ** self.eta
        ...
        are_living_after_bif, inds_after_bif, dt_factor, drs_0 = (
            self.__explicit_network_trim(
                test_network, drs_0, backward_branches, backward_bifurcations, BEA_step
            )
        )
        ...
        # APPROXIMATION LOOP
        while (
            test_network.active_branches
            and approximation_step < self.max_approximation_step
            and not (~are_living_after_bif[:, 0]).all()
        ):
            ...
            drs[are_living_after_bif[:, 0]] = (
                drs_0[are_living_after_bif[:, 0]] + drs_1[inds_after_bif]
            ) / 2
            velocity = drs / dt_0
        ...
        initial_network.active_branches = [
            b
            for i, b in enumerate(network.active_branches)
            if are_living_after_bif[i, 1]
        ]
