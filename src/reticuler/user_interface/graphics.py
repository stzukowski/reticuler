...
def plot_tree(
    ...
    **kwargs_tree_plot,
    ...
    if is_thick:
        ...
        thick_tree = tree.buffer(
            distance=system.extender.pde_solver.finger_width / 2,
            cap_style=1,
            join_style=1,
            resolution=99,
        )
        ...
        plot_polygon(
            ...
            edgecolor="0",
            facecolor=options_tree_plot["color"],
        )
    # for branch in system.network.branches:
    #     line = branch.points
    ...
    ymax_tree = np.min(
        (np.max(system.network.box.points[:, 1]), ymax_tree)
    )  # np.max(system.network.box.points[:,1])*1.05
