...
def plot_tree(ax, system, 
    ...
    # PLOT BOX
    points_to_plot = system.network.box.points[system.network.box.connections]
    ...
    for branch in system.network.branches:
        ...
        if not isThick:
            ...
            contour = np.vstack(contours[0:3])
            ax.fill(contour[:,0], contour[:,1], 
                         color=options_tree_plot["color"], 
                         lw=0, transform=rot+base)
            ax.plot(*contour.T, **options_tree_plot, transform=rot+base) # ,marker=".",ms=5
...
def animate_tree(system0, ylim=None, xlim=None, rot_angle=None, max_time=None, speed_factor=None, **kwargs_tree_plot):
    """Plot a tree with optional arguments from \*\*kwargs_tree, and a black box.
