...
def clip_to_step(system, max_step):
    ...
    for branch in branches_to_iterate:
        ...
        if sum(to_trash) + 1 >= len(branch.steps):  # delete whole branch
            ...
            system.network.branch_connectivity = system.network.branch_connectivity[
                mask, ...
            ]

            mask = (
                system.network.branch_connectivity[:, 0] != branch.ID
    ...
    system.growth_gauges[1], system.growth_gauges[2] = (
        system.network.height_and_length()
    )
    ...
    if type(system.morpher).__name__ == "Jellyfish":
        ...
        R_rim0 = (
            system.network.box.points[:, 0].min()
            + system.network.box.points[:, 0].max()
        ) / 2
        beta = system.morpher.radii[-1] / R_rim0
        system.network.box.points = extend_radially(
            system.network.box.points, R_rim0, beta
        )
    ...
    if type(system.extender.pde_solver).__name__ == "FreeFEM_ThinFingers_Boundary":
        system.extender.pde_solver.box_history = system.extender.pde_solver.box_history[
            : int(max_step + 1)
        ]
...
def clip_to_length(system, max_length):
    ...
    branches_to_iterate = system.network.branches.copy()
    for branch in branches_to_iterate:
        length_throughout_evolution[branch.steps[1:]] = length_throughout_evolution[
            branch.steps[1:]
...
def clip_to_BEA_step(backward_system, max_BEA_step):
    def update_bifs():
        ind_bifurcation = (
            backward_system.backward_bifurcations.mother_IDs
    ...
    for i, forward_branch in enumerate(branches_to_iterate):
        ...
        if len(backward_branch.steps):
            ...
            if ind_backward < 0:
                ...
                backward_system.backward_branches[forward_branch.ID] = BackwardBranch(
                    backward_branch.ID, backward_branch.mother_ID
                )
                # and update backward_bifurcations
                update_bifs()
                max_forward_step_found = max(
                    max_forward_step_found, max(forward_branch.steps)
                )
            else:
                max_BEA_step_found = max(
                    max_BEA_step_found,
                ...
                if num_left == 0:
                    ...
                    mask = (
                        backward_system.system.network.branch_connectivity[:, 1]
                        != forward_branch.ID
                    )
                    backward_system.system.network.branch_connectivity = (
                        backward_system.system.network.branch_connectivity[mask, ...]
                    )
                    ...
                    backward_branch.nums_left = backward_branch.nums_left[
                        : ind_backward + 1
                    ]
                    backward_branch.steps = backward_branch.steps[: ind_backward + 1]
                    backward_branch.flux_info = backward_branch.flux_info[
                        : ind_backward + 1
                    ]
                    backward_branch.overshoot = backward_branch.overshoot[
                        : ind_backward + 1
                    ]
                    backward_branch.angular_deflection = (
                        backward_branch.angular_deflection[: ind_backward + 1]
                    )
                    # and update backward_bifurcations
                    update_bifs()
    ...
    backward_system.system.timestamps = backward_system.system.timestamps[
        : max_forward_step_found + 1
    ]
    backward_system.system.growth_gauges[0] = max_forward_step_found
    backward_system.system.growth_gauges[1], backward_system.system.growth_gauges[2] = (
        backward_system.system.network.height_and_length()
    )
    backward_system.system.growth_gauges[3] = backward_system.system.timestamps[-1]

    backward_system.backward_bifurcations.flags[
        backward_system.backward_bifurcations.flags == 4
    ] = 1
...
if __name__ == "__main__":
    ...
    backward_system = BackwardSystem.import_json(
        input_file=dir_name + "eta00_back_0", system=system
    )
    ...
    logger.info(
        "backward_branches[-1].steps: %s", backward_system.backward_branches[-1].steps
    )
