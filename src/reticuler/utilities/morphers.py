...
class Jellyfish:
    ...
    def morph(self, network, out_growth, step):
        ...
        Ps = ds * sigmoid(
            distances_ang * R_rim0,
            sig_shift=self.sprouting_thresh,
            sig_rate=self.sprouting_sig_rate,
            sig_h=self.sprouting_sig_h,
        )
        ...
        if len(network.active_branches) == 0 and not any(is_triggered):
            ...
            while not any(is_triggered):
                R_rim1 = R_rim0 + dr
                beta = R_rim1 / R_rim0
                new_radii, R_rim0, distances_ang, mid_pos_ang = (
                    extend_box_check_distances()
                )
            ...
            step = step + 1
            logger.warning(
                "No sprouts and no space for new ones. Additional jellyfish growth."
            )
        ...
        for i, theta in enumerate(mid_pos_ang[is_triggered]):
            theta = (
                ...
                * self.sprouting_stochastic_shift
                / R_rim0
            )
            branch = Branch(
                ID=max_branch_id + i + 1,
                points=np.vstack(
                    cyl2cart(np.array([R_rim0, R_rim0 - 0.075]), theta, R_rim0)
                ),
            ...
            _, ind_min, is_pt_new, _, ind_min_end = find_reconnection_point(
                seed,
                network.box.points,
            ...
            if not is_pt_new:
                ...
                network.box.points = np.insert(
                    network.box.points, ind_min + 1, seed, axis=0
                )
                network.box.connections = np.vstack(
                    (network.box.connections, [network.box.connections[-1, 0] + 1, 0])
                )
                network.box.connections[-2, 1] = network.box.connections[-1, 0]
                network.box.boundary_conditions = np.append(
                    network.box.boundary_conditions, network.box.boundary_conditions[-1]
                )

            network.box.seeds_connectivity[
                network.box.seeds_connectivity[:, 0] > ind_min, 0
            ] = (
                network.box.seeds_connectivity[
                    network.box.seeds_connectivity[:, 0] > ind_min, 0
                ]
            ...
            network.box.seeds_connectivity = np.vstack(
                (network.box.seeds_connectivity, [ind_min + 1, branch.ID])
            )

        if any(is_triggered):
            thetas = [f"{t:.3f}" for t in mid_pos_ang[is_triggered] / np.pi * 180]
            logger.info(
                "Initiating %d new sprouts at theta=%s deg.",
                sum(is_triggered),
                ", ".join(thetas),
            )
