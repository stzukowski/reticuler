...
class Box:
    ...
    def __init__(
        ...
        self.connections = (
            np.empty((0, 2), dtype=int) if connections is None else connections
        )
        ...
        self.seeds_connectivity = (
            [] if seeds_connectivity is None else seeds_connectivity
        )
    ...
    def construct(cls, initial_condition=100, **kwargs_construct):
        ...
        if initial_condition // 100 == 1 or initial_condition // 30 == 10:
            ...
            if type(options_construct["seeds_x"]) == int:
                options_construct["seeds_x"] = (
                    options_construct["width"]
                    / options_construct["seeds_x"]
                    * (np.arange(options_construct["seeds_x"]) + 0.5)
                )
            else:
                options_construct["seeds_x"] = np.array(options_construct["seeds_x"])

            if not len(options_construct["initial_lengths"]) == len(
                options_construct["seeds_x"]
            ):
            ...
            if not len(options_construct["branch_BCs"]) == len(
                options_construct["seeds_x"]
            ):
            ...
            if initial_condition // 100 == 1:
                ...
                # left boundary
                box.__add_points([[0, options_construct["height"]], [0, 0]])
                ...
                if initial_condition == 301:
                    ...
                    box.__add_points(
                        np.vstack(
                            (
                                options_construct["width"]
                                * np.cos(np.linspace(0, np.pi, n_points_top)),
                                options_construct["height"]
                                * np.sin(np.linspace(0, np.pi, n_points_top)),
                            )
                        ).T
                    )
                if initial_condition == 300:
                    ...
                    box.__add_points(
                        np.vstack(
                            [
                                np.linspace(
                                    options_construct["width"], 0, n_points_top
                                ),  # array of x
                                options_construct["height"]
                                * np.ones(n_points_top),  # array of y (constant)
                            ]
                        ).T
                    )
                    # bottom left corner
            ...
            box.seeds_connectivity = np.column_stack(
                (
                    2 + np.arange(sum(mask_seeds_from_outlet)),
            ...
            if initial_condition in [103, 300]:  # or top DIRICHLET_1:
                box.boundary_conditions[1 : -2 - sum(~mask_seeds_from_outlet)] = (
                    DIRICHLET_1
                )
            if initial_condition == 301:  # or top DIRICHLET_1 and bottom no flux
                box.boundary_conditions[: -1 - sum(~mask_seeds_from_outlet)] = (
                    DIRICHLET_1
                )
                box.boundary_conditions[-1 - sum(~mask_seeds_from_outlet) :] = NEUMANN_0
                        ...
                        points=np.array(
                            [
                                ...
                                [
                                    x,
                                    options_construct["height"]
                                    - options_construct["initial_lengths"][i],
                                ],
                            ]
                        ),
                        steps=np.array([0, 0]),
        ...
        # Jellyfish - octant
            ...
            options_construct.update(kwargs_construct)
            if (
                not len(options_construct["branch_BCs"])
                == len(options_construct["seeds_phi"]) + 3
            ):
                options_construct["branch_BCs"] = options_construct["branch_BCs"] * (
                    len(options_construct["seeds_phi"]) + 3
                )
            ...
            box.__add_points(
                cyl2cart(
                    R_stom,
                    np.linspace(
                        angular_width / 2, -angular_width / 2, n_points_stomach + 1
                    ),
                    R_rim,
                )
            )
            ...
            rim_pts_angs = np.linspace(
                -angular_width / 2, angular_width / 2, n_points_rim + 1
            )[:-1]
            box.__add_points(cyl2cart(R_rim, rim_pts_angs, R_rim))
            ...
            box.seeds_connectivity = np.column_stack(
                (
                    [
                        ...
                        n0_rim + n_points_rim // 4,
                        n0_rim + n_points_rim // 4 * 3,
                        n0_rim + n_points_rim // 8,
                        n0_rim + n_points_rim // 8 * 3,
                        n0_rim + n_points_rim // 8 * 5,
                        n0_rim + n_points_rim // 8 * 7,
                    ],
                    np.arange(7),
                )
            )
            ...
            if initial_condition == 200:
                box.boundary_conditions[1 : n_points_stomach + 1] = (
                    DIRICHLET_0_GLOB_FLUX  # top
                )
                ...
                raise ValueError(
                    f"Jelly IC {initial_condition} is incorrect! Choose another one."
                )
            ...
            r1 = np.sqrt(
                (2 * R_rim * np.sin(angular_width / 8)) ** 2
                - (2 / 3 * h0 * np.sin(angular_width / 16)) ** 2
            ) / np.cos(angular_width / 16)
            x = R_rim - r1 * np.cos(t)
            y = 2 / 3 * h0 * np.sin(t)
            branches.append(
                Branch(
                    ID=1,
                    points=np.vstack(
                        (box.points[n0_rim + n_points_rim // 4], np.vstack((x, y)).T)
                    ),
            ...
            branches.append(
                Branch(
                    ...
                    points=np.vstack(
                        (
                            box.points[n0_rim + n_points_rim // 4 * 3],
                            np.vstack((x, y)).T,
                        )
                    ),
                    steps=np.zeros(n_trifork + 1),
        ...
        # Leaf radial seeds in the center: circle or slice
            ...
            if (
                type(options_construct["seeds_phi"]) == int
                and box.initial_condition == 350
            ):
                options_construct["seeds_phi"] = (
                    2
                    * np.pi
                    / options_construct["seeds_phi"]
                    * np.arange(options_construct["seeds_phi"])
                )
            elif (
                type(options_construct["seeds_phi"]) == int
                and box.initial_condition == 351
            ):
                ang_dist = (
                    options_construct["angular_width"] / options_construct["seeds_phi"]
                )
                options_construct["seeds_phi"] = np.arange(
                    -options_construct["angular_width"] / 2 + ang_dist / 2,
                    options_construct["angular_width"] / 2,
                    ang_dist,
                )
            ...
            n_points_rim = 1 + int(
                1
                / 0.02
                * options_construct["radius"]
                * options_construct["angular_width"]
            )
            ...
            for i, phi in enumerate(options_construct["seeds_phi"]):
                ...
                branch = Branch(
                    ID=i,
                    points=np.array([[0, 0], [IL * np.sin(phi), IL * np.cos(phi)]]),
                    steps=np.array([0, 0]),
                    BC=BC,
                )
                branches.append(branch)
            ...
            raise ValueError(
                f"Initial condition {initial_condition} is incorrect! Choose another one."
            )
