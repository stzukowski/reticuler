...
class Network:
    ...
    def __init__(
        ...
        self.active_branches = (
            [] if active_branches is None else active_branches
        )  # moving branches (to extend)
    ...
    def reconnect(self, pde_solver, step):
        ...
        if "FreeFEM_ThickFingers" in type(pde_solver).__name__:
            ...
            reconnection_distance_bt = (
                pde_solver.finger_width / 2 + 5e-3
            )  # 0.05*pde_solver.ds
        else:
        ...
        def index_branches():
            ...
            for i, branch in enumerate(self.branches):
                ...
                all_segments_branches = np.vstack(
                    (
                        all_segments_branches,
                        np.column_stack(
                            (
                                np.ones(n_points - 1) * branch.ID,
        ...
        def index_outlet():
            mask_outlet = (
                (self.box.boundary_conditions == DIRICHLET_0_GLOB_FLUX)
            ...
            all_segments_outlet = np.column_stack(
                (inds_outlet, pts_outlet[:, 0], pts_outlet[:, 1])
            )
        ...
        for branch in branches_to_iterate:
            ...
            if min_distance < reconnection_distance_bt:
                ...
                if False and pde_solver.ds >= 1e-5:
                    ...
                    logger.info(
                        "! Branch %d is reaching the outlet ! ds = %s",
                        branch.ID,
                        pde_solver.ds,
                    )
                else:
                    ...
                    if is_pt_new:
                        ...
                        self.box.points = np.insert(
                            self.box.points, ind_new_pt, breakthrough_pt, axis=0
                        )
                        mask_temp = self.box.connections >= ind_new_pt
                        self.box.connections[mask_temp] = (
                            self.box.connections[mask_temp] + 1
                        )
                        new_conns = self.box.connections[ind_new_conn]
                        new_conns = [
                            [new_conns[0], ind_new_pt],
                            [ind_new_pt, new_conns[1]],
                        ]
                        self.box.connections = np.vstack(
                            (
                                self.box.connections[:ind_new_conn],
                                new_conns,
                                self.box.connections[ind_new_conn + 1 :],
                            )
                        )

                        self.box.boundary_conditions = np.insert(
                        ...
                        self.box.seeds_connectivity[mask_temp, 0] = (
                            self.box.seeds_connectivity[mask_temp, 0] + 1
                        )
            ...
            # RECONNECTION TO OTHER BRANCHES
                ...
                len(self.branches) > 1
            ):  # and branch.length() > 2*reconnection_distance:
                mask = np.ones(len(all_segments_branches), dtype=bool)
                far_from_tip = sum(
                    np.cumsum(
                        np.flip(
                            np.linalg.norm(
                                branch.points[1:] - branch.points[:-1], axis=1
                            )
                        )
                    )
                    > 1.05 * reconnection_distance
                )
                mask_branch = np.logical_and(
                    all_segments_branches[:, 0] == branch.ID,
                    all_segments_branches[:, 2] >= far_from_tip - 1,
                )
                ...
                if min_distance < reconnection_distance:
                    ...
                    logger.info(
                        "! Branch %d reconnected to branch %d !", branch.ID, branch2_id
                    )
                    ...
                    if is_pt_new:
                        ...
                        ind_pt = int(
                            all_segments_branches[mask][ind_min, 2]
                        )  # ind_min is the index of the closest segment
                        branch2.points = np.insert(
                            branch2.points, ind_pt + 1, reconnection_pt, axis=0
                        )
                        branch2.steps = np.insert(
                            branch2.steps, ind_pt + 1, branch2.steps[ind_pt + 1], axis=0
                        )

                        # update all_segments_branches (in case something else reconnects to the same branch)
                        all_segments_branches = all_segments_branches[
                            all_segments_branches[:, 0] != branch2_id
                        ]
