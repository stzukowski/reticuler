...
class Streamline:
    ...
    def __init__(self, pde_solver,
                 eta=1.0, ds=0.01,
                 bifurcation_type=0, bifurcation_thresh=0, bifurcation_angle=2*np.pi/5):
        ...
        self.inflow_thresh = 0.05 # less than 5% of max flux/velocity puts branches asleep
    ...
    def check_bifurcation_and_moving_conditions(self, network):
        ...
        for i, branch in enumerate(network.active_branches):
            ...
            if self.bifurcation_type and branch.length()>self.distance_from_bif_thresh:
            ...
            if a1 < self.inflow_thresh * max_a1 or \
                a1**self.eta < self.inflow_thresh * max_a1**self.eta:
    ...
    def find_test_dRs(self, network):
        ...
        for i, branch in enumerate(network.active_branches):
            ...
            dRs_test[i] = np.dot( self.__rotation_matrix(tip_angle), \
                            self.__streamline_extension(beta, dr) )
        return dRs_test  
