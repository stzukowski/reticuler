...
class ModifiedEulerMethod:
    ...
    def __init__(self, pde_solver, is_reconnecting=False, max_approximation_step=1):
    ...
    def integrate(self, network, step, is_dr_normalized=True):
        ...
        out_solver = self.pde_solver.solve_PDE(
            network
        )  # returns flux_info_0 for backward and rim_xy_fluxes for leaf
        ...
        while approximation_step < self.max_approximation_step and not did_reconnect:
            ...
            if is_dr_normalized:
                dRs_test = (
                    self.pde_solver.ds
                    * dRs_test
                    / np.max(np.linalg.norm(dRs_test, axis=1))
                )
                ...
                dt = np.mean(
                    np.linalg.norm(dRs_test, axis=1)
            ...
            for i, branch in enumerate(network.active_branches):
                branch.points[-1] = (
                    network1.active_branches[i].points[-1] - dRs_0[i] + dRs_test[i]
                )
            ...
            if self.is_reconnecting:
                ...
                if did_reconnect:
                    logger.info(
                        "Reconnected branch, (possibly) skipping Modified Euler Method steps."
                    )
                    break
...
class ModifiedEulerMethod_Boundary(ModifiedEulerMethod):
    ...
    def integrate(self, network, step, is_dr_normalized=True):
        ...
        mask_moving_boundary[1:] |= mask_moving_boundary[
            :-1
        ].copy()  # "spill" True on point i+1 (i-last detected)
        ...
        if did_reconnect:
            ...
            while approximation_step < self.max_approximation_step:
                ...
                dRs_boundary_test = (
                    1 - alpha
                ) * dRs_boundary_0 + alpha * dRs_boundary_1
                ...
                network.box.points[mask_moving_boundary] = (
                    network1.box.points[mask_moving_boundary]
                    - dRs_boundary_0
                    + dRs_boundary_test
                )
                # print('Forth loop, approximation step: {step}.'.format(step=approximation_step) )
        ...
        self.pde_solver.control_point_density(
            network, network.box.points[mask_moving_boundary]
        )
