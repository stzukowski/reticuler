...
class FreeFEM:
    ...
    def prepare_contour(
        self, border_contour, inside_buildmesh, i, points, label, border_name="contour"
    ):
        for j, pair in enumerate(zip(points, points[1:])):
            ...
            border_contour = (
                ...
                + "border {b_n}{i}connection{j}(t=0, 1){{x={x0:.6e}+t*({ax:.6e});y={y0:.6e}+t*({ay:.6e}); label={label};}}\n".format(
                    ...
                    y0=y0,
                    ay=y1 - y0,
                    label=label[j] if not np.isscalar(label) else label,
                )
            )

            inside_buildmesh = inside_buildmesh + " {b_n}{i}connection{j}(1) +".format(
                b_n=border_name, i=i, j=j
            )

        return border_contour, inside_buildmesh

    def prepare_contour_list(
        self,
        border_contour,
        inside_buildmesh,
        i,
        ...
        if not np.isscalar(label):
            border_contour = (
                border_contour + f" int[int] {border_name}{i}BC({n_points-1});"
            )
        ...
        inside_buildmesh = inside_buildmesh + f" {border_name}{i}({border_name}{i}N) +"
        return border_contour, inside_buildmesh
    ...
    def prepare_script_box(self, connections_bc, points, points_per_unit_len=0.5):
        ...
        ns_border = np.max(
            (
                np.ones(connections_bc.shape[0]),
                points_per_unit_len
                * np.linalg.norm(
                    np.diff(points[connections_bc[:, :2]], axis=1)[:, 0], axis=1
                ),
            ),
            axis=0,
        ).astype(int)
        ...
        border_box, inside_buildmesh_box = self.prepare_contour_list(
            border_box,
            inside_buildmesh="",
            i="",
            points=np.vstack((points, points[0])),
            label=connections_bc[:, 2],
            ns_border=ns_border,
            border_name="box",
        )
    ...
    def run_freefem(self, script):
        ...
        if self.is_script_saved:
            ...
            script_name = (
                ...
                + f"_{datetime.now().strftime('%Y_%m_%d-%p%I_%M_%S')}.edp"
            )
        ...
        if out_freefem.returncode or "nan" in out_freefem.stdout.decode():
            script_name = (
                self.system.exp_name
                + f"_{datetime.now().strftime('%Y_%m_%d-%p%I_%M_%S')}_failed.edp"
            )
