...
class BackwardBranch:
    ...
    active_ind : int, default -1
        Index in the system.network.active_branches list. -1 if not there.
    sleeping_ind : int, default -1
        Index in the system.network.sleeping_branches list. -1 if not there.
    points : array
    ...
    def __init__(self, ID, mother_ID):
        ...
        self.active_ind = -1 # index in the system.network.active_branches list (-1 == not there)
        self.sleeping_ind = -1 # index in the system.network.sleeping_branches list (-1 == not there)
...
class BackwardSystem:
    ...
    def import_json(cls, input_file, system):
        ...
        for i in reversed(list(json_load["backward_branches"].keys())):
            ...
            backward_branch.active_ind = json_branch["active_ind"]
            backward_branch.sleeping_ind = json_branch["sleeping_ind"]
    ...
    def export_json(self):
        ...
        for i, active_branch in enumerate(self.system.network.active_branches):
            self.backward_branches[active_branch.ID].active_ind = i
        for i, sleeping_branch in enumerate(self.system.network.sleeping_branches):
            self.backward_branches[sleeping_branch.ID].sleeping_ind = i
        ...
        for branch in self.backward_branches[::-1]:
            branch_dict = {
                branch.ID: {
                    ...
                    "active_ind": branch.active_ind,
                    "sleeping_ind": branch.sleeping_ind,
