...
class BackwardSystem:
    ...
    def import_json(cls, input_file, system):
        ...
        for i in reversed(list(json_load["backward_branches"].keys())):
            ...
            if np.asarray(json_branch["points_and_steps"]).size:
                backward_branch.points = np.asarray(json_branch["points_and_steps"])[:, :2]
                backward_branch.steps = np.asarray(json_branch["points_and_steps"])[:, 2]
