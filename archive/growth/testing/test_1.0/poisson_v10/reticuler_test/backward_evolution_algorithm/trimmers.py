...
class BackwardModifiedEulerMethod:
    ...
    def __init__(
        ...
        self.inflow_thresh = 0.05
        
    def trim(self, network, backward_branches, backward_bifurcations, BEA_step):
        ...
        return initial_network, test_network, dt        
    ...
    def __explicit_network_trim(self, test_network, drs, backward_branches, backward_bifurcations, BEA_step):
        ...
        are_living_after_bif = np.empty(len(branches_to_iterate), dtype=bool)
        for i, forward_branch in enumerate(branches_to_iterate):
            ...
            # bifurcation reached
                ...
                # bifurcation point
                    ...
                    if backward_bifurcations[ind_bifurcation].flag == 0:
                        backward_bifurcations[ind_bifurcation].flag = 1
                        ...
                        inds_remaining = [i for i, b in enumerate(test_network.active_branches) if b.ID in remaining_IDs]
                    ...
                    elif backward_bifurcations[ind_bifurcation].flag == 1:
                        backward_bifurcations[ind_bifurcation].flag = 2
    ...
    def __calculate_remaining_length(self, network, mother_ID):
        to_check = np.array([mother_ID])
        branch_IDs = np.array([b.ID for b in network.branches])
        remaining_length = 0
        remaining_IDs = []
        while to_check:
            next_ID = to_check[0]
            remaining_length = remaining_length + network.branches[branch_IDs==next_ID].length()
            
            to_add = network.branch_connectivity[network.branch_connectivity[:,1]==next_ID]
            if to_add:
