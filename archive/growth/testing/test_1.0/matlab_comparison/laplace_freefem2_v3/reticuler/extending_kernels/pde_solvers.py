...
class FreeFEM:
    ...
    def __prepare_script(self, network):
        ...
        for i, triple in enumerate(network.box.connections_bc()):
            ...
            border_box = (
                border_box
                + "border box{i}(t=0, 1){{x={x0:.12f}+t*({ax:.12f});y={y0:.12f}+t*({ay:.12f}); label={bc:.12f};}}\n".format(
        ...
        for i, branch in enumerate(network.branches):
            for j, pair in enumerate(zip(branch.points, branch.points[1:])):
                ...
                border_network = (
                    border_network
                    + "border branch{i}connection{j}(t=0, 1){{x={x0:.12f}+t*({ax:.12f});y={y0:.12f}+t*({ay:.12f}); label=1;}}\n".format(
        ...
        for i, branch in enumerate(network.active_branches):
            tip_information = (
                tip_information
                + "\nX({j})={x:.12f};".format(j=i, x=branch.points[-1, 0])
                + "\nY({j})={y:.12f};".format(j=i, y=branch.points[-1, 1])
                + "\nangle({j})={angle:.12f};".format(
