...
class FreeFEM:
    ...
    def __prepare_script(self, network):
        ...
        for i, triple in enumerate(network.box.connections_bc()):
            ...
            border_box = (
                border_box
                + "border box{i}(t=0, 1){{x={x0:.12e}+t*({ax:.12e});y={y0:.12e}+t*({ay:.12e}); label={bc:.12e};}}\n".format(
        ...
        for i, branch in enumerate(network.branches):
            for j, pair in enumerate(zip(branch.points, branch.points[1:])):
                ...
                border_network = (
                    border_network
                    + "border branch{i}connection{j}(t=0, 1){{x={x0:.12e}+t*({ax:.12e});y={y0:.12e}+t*({ay:.12e}); label=1;}}\n".format(
        ...
        for i, branch in enumerate(network.active_branches):
            tip_information = (
                tip_information
                + "\nX({j})={x:.12e};".format(j=i, x=branch.points[-1, 0])
                + "\nY({j})={y:.12e};".format(j=i, y=branch.points[-1, 1])
                + "\nangle({j})={angle:.12e};".format(
    ...
    def solve_PDE(self, network):
        ...
        with np.printoptions(formatter={'float': '{:.10e}'.format}):
            print('a1a2a3: \n', self.a1a2a3_coefficients)