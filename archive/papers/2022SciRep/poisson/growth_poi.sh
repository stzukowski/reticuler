...
ds=0.001 # spatial step

# growth limit (when to stop the simulation) 
growthThreshType=2 # 0: number of steps, 1: height, 2: network length 3: time
growthThresh=9 # Poisson: 13
...
# wait_till 2 # ( ./growth_poi.sh > ~/reticuler/bash_scripts/out_poi.txt 2>&1 & )
echo "Starting growth."
for subexp in papers/2022SciRep/poisson
do
	...
	cp -ar ~/reticuler/bash_scripts/growth_poi.sh ~/$experimentDirectory/
	
	for iEta in 15 # {0..30..10}
	do
		run_single_exp $iEta
	done
done
